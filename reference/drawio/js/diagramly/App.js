...
(function()
{
	...
	App.prototype.setMode = function(mode, remember)
	{
		...
		if (this.appIcon != null)
		{
			...
			else if (mode == App.MODE_MAD)
			{
				this.appIcon.setAttribute('title', mxResources.get('openIt', [mxResources.get('mad')]));
				this.appIcon.style.cursor = 'pointer';
			}
			else
			{
...
App.prototype.saveLibrary = function(name, images, file, mode, noSpin, noReload, fn)
{
	try
	{
		...
		if (file == null & mode == App.MODE_MAD)
		{
			file = new MadLibrary(this, xml, name);
		}
				...
				if (name != null && name.length > 0)
				{
					...
					else
					{
						...
						else if (mode == 'mad')
						{
							...
							var tmp = new MadFile(this.currentFile.ui, this.currentFile.data, meta);
							tmp.save();
						}
						else if (prev != mode)
						{
...
App.prototype.createFile = function(title, data, libs, mode, done, replace, folderId, tempFile, clibs, success)
{
	...
	if (title != null && this.spinner.spin(document.body, mxResources.get('inserting')))
	{
		...
		try
		{
			...
			else if (mode == App.MODE_MAD && this.mad != null)
			{
				this.mad.insertFile(title, data, fileCreated, error);
			}
...
App.prototype.getLibraryStorageHint = function(file)
{
	...
	else if (file.constructor == MadLibrary)
	{
		tip += ' (' + mxResources.get('mad') + ')';
	}
...
App.prototype.exportFile = function(data, filename, mimeType, base64Encoded, mode, folderId)
{
	...
	{
		if (this.mad != null && this.spinner.spin(document.body, mxResources.get('saving')))
		{
			// LATER: Add folder picker
			this.mad.insertFile(filename, (base64Encoded) ? this.base64ToBlob(data, mimeType) :
				data, mxUtils.bind(this, function()
			{
				this.spinner.stop();
			}), mxUtils.bind(this, function(resp)
