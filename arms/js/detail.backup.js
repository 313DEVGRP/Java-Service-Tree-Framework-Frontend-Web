	...
	$(jQueryElementID)
		...
		.bind("select_node.jstree", function (event, data) {
			...
			if ($.isFunction(detailTreeClick)) {
				...
				console.log(
					"[ detailTreeBuild :: select_node ] :: data.rslt.obj.children('ul')" + data.rslt.obj.children("ul")
				);
...
function bindPagination(curPage, totalPages, funName) {
	...
	if (curPage > 1 && pageLimit < curPage) {
		pageUrl +=
			"<li class='page-item'><a class='first_page' href='javascript:" +
			funName +
			"(1);'><i class='fa fa-angle-double-left' /></a></li>";
	}
	//이전 페이지
	if (curPage > pageLimit) {
		pageUrl +=
			"<li class='page-item'><a class='prev_page' href='javascript:" +
			funName +
			"(" +
			(startPage == 1 ? 1 : startPage - 1) +
			");'><i class='fa fa-angle-left' /></a></li>";
	}
	...
	if (nextPage <= totalPages) {
		pageUrl +=
			"<li class='page-item'><a class='next_page' href='javascript:" +
			funName +
			"(" +
			(nextPage < totalPages ? nextPage : totalPages) +
			");'><i class='fa fa-angle-right' /></a></li>";
	}
	//맨 마지막 페이지
	if (curPage < totalPages && nextPage < totalPages) {
		pageUrl +=
			"<li class='page-item'><a class='last_page' href='javascript:" +
			funName +
			"(" +
			totalPages +
...
function createReqCommentList(comment, previousDate) {
	...
	var dateSeparator = showDateSeparator
		? `<div class="date-separator"><i class="bx bx-calendar"></i> ${fullDate.split(" - ")[0]}</div>`
		: "";
	...
	$newHtml = $(`
                    ...
                    <div class="chat-message">
                        ...
                        <div class="chat-contents" style="display: flex; position: relative; flex-direction: column; align-items: ${
													position === "on-left" ? "flex-end" : "flex-start"
												}; width: fit-content; float:${position === "on-left" ? "right" : "left"};">
...
function set_req_list_button() {
	$(".dd-item").each(function () {
		if ($(this).children("ol").length > 0) {
			$(this).prepend(
				'<button data-action="collapse" type="button" class="collapseBtn"  style="display: block;">Collapse</button>' +
